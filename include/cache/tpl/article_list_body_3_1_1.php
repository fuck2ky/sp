...
src="<?php echo $v[0] ?>" itemid="<?php echo $v['i'] ?> " > 
<DIV class=footer_image><?php echo $v[1] ?></DIV></DIV>
<?php } ?>

</DIV></DIV></DIV></DIV>
<DIV class="overlay apple_overlay i_nav"><A class=close 
href=""></A>
<DIV class=contentWrap>
<?php $vn=0;$tablev=syClass("syModel")->syCache(3600)->findSql("select id,tid,sid,title,style,trait,gourl,addtime,hits,htmlurl,htmlfile,litpic,orders,mrank,mgold,isshow,keywords,description,pics,video,bigpic,video1 from dy_article a left join dy_article_field b on (a.id=b.aid) where isshow=1 and tid in(23,37,38)  order by id desc  limit 1");foreach($tablev as $v){ $v["tid_leafid"]=$sy_class_type->leafid($v["tid"]);$v["n"]=$vn=$vn+1; $v["url"]=html_url("article",$v); $v["title"]=stripslashes($v["title"]); $v["description"]=stripslashes($v["description"]); ?>



<object classid="clsid:D27CDB6E-AE6D-11cf-96B8-444553540000" codebase="http://download.macromedia.com/pub/shockwave/cabs/flash/swflash.cab#version=7,0,19,0" width="651" height="425">    
                                ...
                                <param name="FlashVars" value="vcastr_file=<?php echo $v['video'] ?>" />    

                                <embed src="Flvplayer.swf" allowfullscreen="true" flashvars="vcastr_file=<?php echo $v['video'] ?>" quality="high" pluginspage="http://www.macromedia.com/go/getflashplayer" type="application/x-shockwave-flash" width="651" height="425"></embed>    
...
<OL id=secondMenu>
  <?php $ytid=$type[tid];if(!syDB("classtype")->find(array("pid"=>3),null,"tid")){ $ypid=syDB("classtype")->find(array("tid"=>3),null,"pid");$type[tid]=$ypid[pid];} ?><?php $tn=0;$tablet=syClass("syModel")->syCache(3600)->findSql("select tid,molds,pid,classname,gourl,litpic,title,keywords,description,orders,mrank,htmldir,htmlfile,mshow from dy_classtype where  pid='3'  order by orders desc,tid");$type[tid]=$ytid;foreach($tablet as $t){ $t["tid_leafid"]=$sy_class_type->leafid($t["tid"]);$t["n"]=$tn=$tn+1; $t["classname"]=stripslashes($t["classname"]);$t["description"]=stripslashes($t["description"]); $t["url"]=html_url("classtype",$t); ?>
  <LI><A  href="<?php echo $t['url'] ?>"<?php if($t['tid']==23){ ?> class=selected <?php } ?>><?php echo $t['classname'] ?></A></LI>
  <?php } ?>
  <BR clear=all>
</OL>
<!--END SECONDO LEVEL--><!--THIRD LEVEL-->
<OL id=thirdMenu>  
  <?php $ytid=$type[tid];if(!syDB("classtype")->find(array("pid"=>23),null,"tid")){ $ypid=syDB("classtype")->find(array("tid"=>23),null,"pid");$type[tid]=$ypid[pid];} ?><?php $tn=0;$tablet=syClass("syModel")->syCache(3600)->findSql("select tid,molds,pid,classname,gourl,litpic,title,keywords,description,orders,mrank,htmldir,htmlfile,mshow from dy_classtype where  pid='23'  order by orders desc,tid");$type[tid]=$ytid;foreach($tablet as $t){ $t["tid_leafid"]=$sy_class_type->leafid($t["tid"]);$t["n"]=$tn=$tn+1; $t["classname"]=stripslashes($t["classname"]);$t["description"]=stripslashes($t["description"]); $t["url"]=html_url("classtype",$t); ?>
...
<A class=dw_PDF href="#" rel=div.overlay:eq(1)>视频</A>
</DIV>
<DIV id=load_items>
...
src="<?php echo $v[0] ?>" itemid="<?php echo $i ?>" ></A> </SPAN></DIV>
<?php } ?>
