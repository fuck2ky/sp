...
<OL id=secondMenu>
  <?php $vn=0;$tablev=syClass("syModel")->syCache(3600)->findSql("select id,tid,sid,title,style,trait,gourl,addtime,hits,htmlurl,htmlfile,litpic,orders,mrank,mgold,isshow,keywords,description,pics,video,bigpic,video1 from dy_article a left join dy_article_field b on (a.id=b.aid) where isshow=1 and tid in(4) and litpic!=''  order by id asc  limit 3");foreach($tablev as $v){ $v["tid_leafid"]=$sy_class_type->leafid($v["tid"]);$v["n"]=$vn=$vn+1; $v["url"]=html_url("article",$v); $v["title"]=stripslashes($v["title"]); $v["description"]=stripslashes($v["description"]); ?> 
  <LI><A  href="<?php echo $v['url'] ?>"  title="<?php echo $v['title'] ?>" <?php if($v['id']==$article['id']){ ?> class=selected <?php } ?>><?php echo $v['title'] ?></A></LI> 
  <?php } ?>
  <BR clear=all>
</OL>
...
src="<?php echo $v[0] ?>" itemid="<?php echo $v['n'] ?> " ></A> </SPAN></DIV>
<?php } ?>
