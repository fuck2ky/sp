...
<?php $vn=0;$tablev=syClass("syModel")->syCache(3600)->findSql("select id,tid,sid,title,style,trait,gourl,addtime,hits,htmlurl,htmlfile,litpic,orders,mrank,mgold,isshow,keywords,description,pics,video,bigpic from dy_article a left join dy_article_field b on (a.id=b.aid) where isshow=1 and tid in(27)  order by id desc  limit 0,3");foreach($tablev as $v){ $v["tid_leafid"]=$sy_class_type->leafid($v["tid"]);$v["n"]=$vn=$vn+1; $v["url"]=html_url("article",$v); $v["title"]=stripslashes($v["title"]); $v["description"]=stripslashes($v["description"]); ?>
<DIV class=list><A 
href="<?php echo $v['url'] ?>"><IMG 
alt="<?php echo $v['title'] ?>" src="<?php echo $v['litpic'] ?>"> 
<H3><?php echo $v['title'] ?></H3><?php echo $v['description'] ?></A></DIV>
<?php } ?>
</DIV>
...
<DIV class="item i_length"><!-- start /WEBCONTROLS/SP/SPED_ITEM_list.ASCX -->
<?php $vn=0;$tablev=syClass("syModel")->syCache(3600)->findSql("select id,tid,sid,title,style,trait,gourl,addtime,hits,htmlurl,htmlfile,litpic,orders,mrank,mgold,isshow,keywords,description,pics,video,bigpic from dy_article a left join dy_article_field b on (a.id=b.aid) where isshow=1 and tid in(21)  order by id desc  limit 6,3");foreach($tablev as $v){ $v["tid_leafid"]=$sy_class_type->leafid($v["tid"]);$v["n"]=$vn=$vn+1; $v["url"]=html_url("article",$v); $v["title"]=stripslashes($v["title"]); $v["description"]=stripslashes($v["description"]); ?>
<DIV class=list><A 
href="<?php echo $v['url'] ?>"><IMG 
alt="<?php echo $v['title'] ?>" src="<?php echo $v['litpic'] ?>"> 
<H3><?php echo $v['title'] ?></H3><?php echo $v['description'] ?></A></DIV>
