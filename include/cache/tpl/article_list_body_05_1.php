...
<!--END HEADER-->
<!--MAIN CONTENT-->
<DIV style="BACKGROUND-IMAGE: url(<?php echo $type['litpic'] ?>)" id=main_content>
<!--MAIN MENU-->
<!--SECOND LEVEL-->
<DIV id=mainMenu>
<H2><A href="/"><IMG 
...
<?php $vn=0;$tablev=syClass("syModel")->syCache(3600)->findSql("select id,tid,sid,title,style,trait,gourl,addtime,hits,htmlurl,htmlfile,litpic,orders,mrank,mgold,isshow,keywords,description,pics,video,bigpic,video1 from dy_article a left join dy_article_field b on (a.id=b.aid) where isshow=1 and tid in(47)  order by id desc  limit 0,3");foreach($tablev as $v){ $v["tid_leafid"]=$sy_class_type->leafid($v["tid"]);$v["n"]=$vn=$vn+1; $v["url"]=html_url("article",$v); $v["title"]=stripslashes($v["title"]); $v["description"]=stripslashes($v["description"]); ?>
<DIV class=list><A 
