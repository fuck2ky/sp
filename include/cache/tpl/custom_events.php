...
<OL id=secondMenu>
  ...
  <LI><A   href="index.php?file=events.html" class="selected"  >会务活动</A></LI>
  <LI><A   href="index.php?file=partner.html" >合作伙伴列表</A></LI>
...
<H3>会务活动</H3>
<?php $tn=0;$tablet=syClass("syModel")->syCache(3600)->findSql("select tid,molds,pid,classname,gourl,litpic,title,keywords,description,orders,mrank,htmldir,htmlfile,mshow,body from dy_classtype where  tid='27'  order by orders desc,tid limit 1");foreach($tablet as $t){ $t["tid_leafid"]=$sy_class_type->leafid($t["tid"]);$t["n"]=$tn=$tn+1; $t["classname"]=stripslashes($t["classname"]);$t["description"]=stripslashes($t["description"]); $t["url"]=html_url("classtype",$t); ?>
<?php echo $t['body'] ?>
<?php } ?>
<!--STORY ITEM-->
...
<?php $vn=0;$tablev=syClass("syModel")->syCache(3600)->findSql("select id,tid,sid,title,style,trait,gourl,addtime,hits,htmlurl,htmlfile,litpic,orders,mrank,mgold,isshow,keywords,description,pics,video,bigpic,video1 from dy_article a left join dy_article_field b on (a.id=b.aid) where isshow=1 and tid in(27)  order by orders desc,id desc  limit 0,3");foreach($tablev as $v){ $v["tid_leafid"]=$sy_class_type->leafid($v["tid"]);$v["n"]=$vn=$vn+1; $v["url"]=html_url("article",$v); $v["title"]=stripslashes($v["title"]); $v["description"]=stripslashes($v["description"]); ?>
<DIV class=list><A 
