...
<OL id=secondMenu>
  ...
  <LI><A  href="<?php echo $t['url'] ?>"<?php if($t['tid']==21){ ?> class=selected <?php } ?>><?php echo $t['classname'] ?></A></LI>
  <?php } ?>
  <BR clear=all>
</OL>
...
<?php $vn=0;$tablev=syClass("syModel")->syCache(3600)->findSql("select id,tid,sid,title,style,trait,gourl,addtime,hits,htmlurl,htmlfile,litpic,orders,mrank,mgold,isshow,keywords,description,pics,video,bigpic,video1 from dy_article a left join dy_article_field b on (a.id=b.aid) where isshow=1 and tid in(21)  order by orders desc,id desc  limit 0,3");foreach($tablev as $v){ $v["tid_leafid"]=$sy_class_type->leafid($v["tid"]);$v["n"]=$vn=$vn+1; $v["url"]=html_url("article",$v); $v["title"]=stripslashes($v["title"]); $v["description"]=stripslashes($v["description"]); ?>
<DIV class=list><A 
