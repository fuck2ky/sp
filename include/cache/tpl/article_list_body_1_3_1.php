...
href=""></A>
<DIV class=contentWrap>
<?php $vn=0;$tablev=syClass("syModel")->syCache(3600)->findSql("select id,tid,sid,title,style,trait,gourl,addtime,hits,htmlurl,htmlfile,litpic,orders,mrank,mgold,isshow,keywords,description,pics,video,bigpic,video1 from dy_article a left join dy_article_field b on (a.id=b.aid) where isshow=1 and tid in(33)  order by id desc  limit 1");foreach($tablev as $v){ $v["tid_leafid"]=$sy_class_type->leafid($v["tid"]);$v["n"]=$vn=$vn+1; $v["url"]=html_url("article",$v); $v["title"]=stripslashes($v["title"]); $v["description"]=stripslashes($v["description"]); ?>
...
<DIV id=add1 class="overlay apple_overlay i_nav"><A class=close></A><!-- the external content is loaded inside this tag -->
<DIV class=contentWrap>
<DIV id=action_scroll_overlay><A class="prev browse"></A><A 
class="next browse"></A></DIV>
<DIV id=overlay_scroll class=addgallery>
<DIV class=items>
<?php $vn=0;$tablev=syClass("syModel")->syCache(3600)->findSql("select id,tid,sid,title,style,trait,gourl,addtime,hits,htmlurl,htmlfile,litpic,orders,mrank,mgold,isshow,keywords,description,pics,video,bigpic,video1 from dy_article a left join dy_article_field b on (a.id=b.aid) where isshow=1 and tid in(33)  order by id desc  limit 1");foreach($tablev as $v){ $v["tid_leafid"]=$sy_class_type->leafid($v["tid"]);$v["n"]=$vn=$vn+1; $v["url"]=html_url("article",$v); $v["title"]=stripslashes($v["title"]); $v["description"]=stripslashes($v["description"]); ?>
<?php foreach( fileall($v['pics']) as $v2){ ?>
<DIV class="item i_length"><IMG alt="<?php echo $v2[1] ?>" 
src="<?php echo $v2[0] ?>" itemid="<?php echo $v2['i'] ?> " > 
<DIV class=footer_image><?php echo $v2[1] ?></DIV></DIV>
<?php } ?>
<?php } ?>
</DIV></DIV></DIV></DIV>
...
<DIV style="HEIGHT: 137px" class=viewport>
<DIV class=overview>
<?php echo $type['body'] ?>
</DIV></DIV>
<A 
class=dw_PDF 
href="#" 
rel=div.overlay:eq(2)>图库</A><BR><A class=dw_PDF 
href="#" 
rel=div.overlay:eq(1)>视频</A> 
</DIV>
<DIV id=load_items>
