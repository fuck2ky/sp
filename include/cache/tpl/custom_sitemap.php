...
<!--END HEADER-->
<!--MAIN CONTENT-->
<DIV style="BACKGROUND-IMAGE: url(images/slide_22.jpg)" id=main_content>
<!--MAIN MENU-->
<!--SECOND LEVEL-->
<DIV id=mainMenu>
<H2><A href="/"><IMG 
...
<div class="article_content noGallery nogallery">
<div class="article scrollbar_content " id="article">
<H3>网站地图</H3>
<div class="scrollbar" style="height: 164px;">
                                <div class="track" style="height: 164px;">
                                    <div class="thumb" style="top: 30.027027027027025px; height: 21px;">
                                        <div class="end">
                                        </div>
                                    </div>
                                </div>
...
<div class="viewport">
<div class="overview" style="width: 100%; top: -202px;">                              
        <ol class="sitemap">
    	<?php $vn=0;$tablev=syClass("syModel")->syCache(3600)->findSql("select tid,molds,pid,classname,gourl,litpic,title,keywords,description,orders,mrank,htmldir,htmlfile,mshow from dy_classtype where  pid=0  order by orders desc,tid limit 0,3");foreach($tablev as $v){ $v["tid_leafid"]=$sy_class_type->leafid($v["tid"]);$v["n"]=$vn=$vn+1; $v["classname"]=stripslashes($v["classname"]);$v["description"]=stripslashes($v["description"]); $v["url"]=html_url("classtype",$v); ?>
        <li>
        <a href="<?php echo $v['url'] ?>"><?php echo $v['classname'] ?></a>
        <ol>
        	<?php $v1n=0;$tablev1=syClass("syModel")->syCache(3600)->findSql("select tid,molds,pid,classname,gourl,litpic,title,keywords,description,orders,mrank,htmldir,htmlfile,mshow from dy_classtype where  pid='".$v[tid]."'  order by orders desc,tid");foreach($tablev1 as $v1){ $v1["tid_leafid"]=$sy_class_type->leafid($v1["tid"]);$v1["n"]=$v1n=$v1n+1; $v1["classname"]=stripslashes($v1["classname"]);$v1["description"]=stripslashes($v1["description"]); $v1["url"]=html_url("classtype",$v1); ?>
            <li>
             <a href="<?php echo $v1['url'] ?>"><?php echo $v1['classname'] ?></a>
                <ol>
                <?php $v2n=0;$tablev2=syClass("syModel")->syCache(3600)->findSql("select tid,molds,pid,classname,gourl,litpic,title,keywords,description,orders,mrank,htmldir,htmlfile,mshow from dy_classtype where  pid='".$v1[tid]."'  order by orders desc,tid");foreach($tablev2 as $v2){ $v2["tid_leafid"]=$sy_class_type->leafid($v2["tid"]);$v2["n"]=$v2n=$v2n+1; $v2["classname"]=stripslashes($v2["classname"]);$v2["description"]=stripslashes($v2["description"]); $v2["url"]=html_url("classtype",$v2); ?>
                <li>
                <a href="<?php echo $v2['url'] ?>"><?php echo $v2['classname'] ?></a>
                </li>
                <?php } ?>
                <?php $tablea1=syClass("syModel")->syCache(3600)->findSql("select * from dy_$v[molds] where  tid='".$v1[tid]."' ");foreach($tablea1 as $a1){?>
                <li>
                <a href="<?php echo $a1['url'] ?>"><?php echo $a1['title'] ?></a>
                </li>
                <?php } ?>
                </ol>
            </li>
            ...
            <?php $tablea=syClass("syModel")->syCache(3600)->findSql("select * from dy_$v[molds] where  tid='".$v[tid]."' ");foreach($tablea as $a){?>
            <li>
        ...
        <ol class="sitemap">
    	<?php $vn=0;$tablev=syClass("syModel")->syCache(3600)->findSql("select tid,molds,pid,classname,gourl,litpic,title,keywords,description,orders,mrank,htmldir,htmlfile,mshow from dy_classtype where  pid=0  order by orders desc,tid limit 3,3");foreach($tablev as $v){ $v["tid_leafid"]=$sy_class_type->leafid($v["tid"]);$v["n"]=$vn=$vn+1; $v["classname"]=stripslashes($v["classname"]);$v["description"]=stripslashes($v["description"]); $v["url"]=html_url("classtype",$v); ?>
        <li>
        <a href="<?php echo $v['url'] ?>"><?php echo $v['classname'] ?></a>
        <ol>
        	<?php $v1n=0;$tablev1=syClass("syModel")->syCache(3600)->findSql("select tid,molds,pid,classname,gourl,litpic,title,keywords,description,orders,mrank,htmldir,htmlfile,mshow from dy_classtype where  pid='".$v[tid]."'  order by orders desc,tid");foreach($tablev1 as $v1){ $v1["tid_leafid"]=$sy_class_type->leafid($v1["tid"]);$v1["n"]=$v1n=$v1n+1; $v1["classname"]=stripslashes($v1["classname"]);$v1["description"]=stripslashes($v1["description"]); $v1["url"]=html_url("classtype",$v1); ?>
        ...
        <ol class="sitemap">
    	<?php $vn=0;$tablev=syClass("syModel")->syCache(3600)->findSql("select tid,molds,pid,classname,gourl,litpic,title,keywords,description,orders,mrank,htmldir,htmlfile,mshow from dy_classtype where  pid=0  order by orders desc,tid limit 6,3");foreach($tablev as $v){ $v["tid_leafid"]=$sy_class_type->leafid($v["tid"]);$v["n"]=$vn=$vn+1; $v["classname"]=stripslashes($v["classname"]);$v["description"]=stripslashes($v["description"]); $v["url"]=html_url("classtype",$v); ?>
        <li>
        <a href="<?php echo $v['url'] ?>"><?php echo $v['classname'] ?></a>
        <ol>
        	<?php $v1n=0;$tablev1=syClass("syModel")->syCache(3600)->findSql("select tid,molds,pid,classname,gourl,litpic,title,keywords,description,orders,mrank,htmldir,htmlfile,mshow from dy_classtype where  pid='".$v[tid]."'  order by orders desc,tid");foreach($tablev1 as $v1){ $v1["tid_leafid"]=$sy_class_type->leafid($v1["tid"]);$v1["n"]=$v1n=$v1n+1; $v1["classname"]=stripslashes($v1["classname"]);$v1["description"]=stripslashes($v1["description"]); $v1["url"]=html_url("classtype",$v1); ?>
...
<!--END ARTICLE--><!--GALLERY--><!--GALLERY--><!--END CONTENT LOADER--></DIV><!--END CONTENT SIDE--><BR 
clear=all></DIV>
<!--FOOTER-->
<DIV id=footer>
<!--HIGHLIGHTS-->
