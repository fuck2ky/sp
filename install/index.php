...
<form action="index.php?go=2" method="post" name="f" onsubmit="return formChk()">
    ...
    <div class="info">
    	<dl><dt>Mysql地址：</dt><dd><input name="host" type="text" class="int" value="localhost" /></dd><dd class="t">一般无需修改</dd></dl>
...
<?php }?>
    ...
    <div class="info">
        ...
        <dl><dt>目录权限：</dt><dd>uploads <?php if ($uploads){echo $uploads;}else{echo ' <span>检测通过</span>';}?><br />include/backup <?php if ($backup){echo $backup;}else{echo ' <span>检测通过</span>';}?><br />include/cache <?php if ($cache){echo $cache;}else{echo ' <span>检测通过</span>';}?><br />config.php <?php if ($config){echo $config;}else{echo ' <span>检测通过</span>';}?></dd></dl>
        <dl style="margin-bottom:20px;"><dt>&nbsp;</dt><dd><input type="submit" id="submit" value="开始安装" class="btnbig" /></dd></dl>
    </div>
</form>
<?php }
...
if($go==2){
	...
	if((int)$_POST['go_backup']==1){
		...
		$db=array('db','host','port','database','login','password','prefix');
	}
	foreach($db as $v){
		if ($v=='port'){
...
if((int)$_POST['go_backup']==1){
	...
	$db = file('doyo.sql');
	$db[5]=str_ireplace('|-auser-|',$_POST['auser'],$db[5]);
	$db[5]=str_ireplace('|-apass-|',md5(md5($_POST['apass']).$_POST['auser']),$db[5]);
	foreach ($db as  $num =>$v) {
		$v=rtrim($v,'\n');$v=rtrim($v,'\r');
...
$fp=@fopen("$filename", "w");
@fclose($fp);
echo '<dl><dt></dt><dd class="oka">安装全部完成！[请删除install文件夹] <a href="../"><span>[浏览网站]</span></a><a href="../doyo.php"><span>[进入后台]</span></a></dd></dl>'; ?>
</div>
<?php }?>
<div style="clear: both"></div>
</div>
</div>
<div class="bottom"><a href="http://wdoyo.com" target="_blank">DoYo通用建站程序</a> Powered by DoYo! 2.2 © 2010-2012 <a href="http://wdoyo.com" target="_blank">wDoYo.com</a></div>
</body>
</html>