...
href=""></A>
<DIV class=contentWrap>
<?php $vn=0;$tablev=syClass("syModel")->syCache(3600)->findSql("select id,tid,sid,title,style,trait,gourl,addtime,hits,htmlurl,htmlfile,litpic,orders,mrank,mgold,isshow,keywords,description,pics,video,bigpic,video1 from dy_article a left join dy_article_field b on (a.id=b.aid) where isshow=1 and tid in(10,32)  order by id desc  limit 1");foreach($tablev as $v){ $v["tid_leafid"]=$sy_class_type->leafid($v["tid"]);$v["n"]=$vn=$vn+1; $v["url"]=html_url("article",$v); $v["title"]=stripslashes($v["title"]); $v["description"]=stripslashes($v["description"]); ?>
...
<OL id=secondMenu>
  ...
  <LI><A  href="<?php echo $t['url'] ?>"<?php if($t['tid']==10){ ?> class=selected <?php } ?>><?php echo $t['classname'] ?></A></LI>
  <?php } ?>
  <BR clear=all>
</OL>
<!--END SECONDO LEVEL--><!--THIRD LEVEL-->
<OL id=thirdMenu>  
  <?php $ytid=$type[tid];if(!syDB("classtype")->find(array("pid"=>10),null,"tid")){ $ypid=syDB("classtype")->find(array("tid"=>10),null,"pid");$type[tid]=$ypid[pid];} ?><?php $tn=0;$tablet=syClass("syModel")->syCache(3600)->findSql("select tid,molds,pid,classname,gourl,litpic,title,keywords,description,orders,mrank,htmldir,htmlfile,mshow from dy_classtype where  pid='10'  order by orders desc,tid");$type[tid]=$ytid;foreach($tablet as $t){ $t["tid_leafid"]=$sy_class_type->leafid($t["tid"]);$t["n"]=$tn=$tn+1; $t["classname"]=stripslashes($t["classname"]);$t["description"]=stripslashes($t["description"]); $t["url"]=html_url("classtype",$t); ?>
...
<DIV style="HEIGHT: 147px" class=viewport>
<DIV class=overview>
<?php echo $type['body'] ?>
</DIV></DIV>
