...
<OL id=thirdMenu>  
  <?php $ytid=$type[tid];if(!syDB("classtype")->find(array("pid"=>$type[tid]),null,"tid")){ $ypid=syDB("classtype")->find(array("tid"=>$type[tid]),null,"pid");$type[tid]=$ypid[pid];} ?><?php $tn=0;$tablet=syClass("syModel")->syCache(3600)->findSql("select tid,molds,pid,classname,gourl,litpic,title,keywords,description,orders,mrank,htmldir,htmlfile,mshow from dy_classtype where  pid='".$type[tid]."'  order by orders desc,tid");$type[tid]=$ytid;foreach($tablet as $t){ $t["tid_leafid"]=$sy_class_type->leafid($t["tid"]);$t["n"]=$tn=$tn+1; $t["classname"]=stripslashes($t["classname"]);$t["description"]=stripslashes($t["description"]); $t["url"]=html_url("classtype",$t); ?>
  <LI><A  href="<?php echo $t['url'] ?>"<?php if($type['tid']==$t['tid']){ ?> class=selected <?php } ?>><?php echo $t['classname'] ?></A></LI>
  <?php } ?>
  <BR clear=all>
</OL>
<DIV class="article_content generic_list">
<H3><?php echo $type['classname'] ?></H3>
<?php echo $type['body'] ?>
<!--STORY ITEM-->
<DIV id=generic_list_side><A class="browse prev"></A><A class="browse next"></A>
<DIV id=story class=scrollable>
<DIV class=items>
<DIV class="item i_length">
<?php $vn=0;$tablev=syClass("syModel")->syCache(3600)->findSql("select id,tid,sid,title,style,trait,gourl,addtime,hits,htmlurl,htmlfile,litpic,orders,mrank,mgold,isshow,keywords,description,pics,video,bigpic,video1 from dy_article a left join dy_article_field b on (a.id=b.aid) where isshow=1 and tid in(34)  order by id asc  limit 0,3");foreach($tablev as $v){ $v["tid_leafid"]=$sy_class_type->leafid($v["tid"]);$v["n"]=$vn=$vn+1; $v["url"]=html_url("article",$v); $v["title"]=stripslashes($v["title"]); $v["description"]=stripslashes($v["description"]); ?>
<DIV class=list><A 
href="<?php echo $v['url'] ?>"><IMG 
alt="<?php echo $v['title'] ?>" src="<?php echo $v['litpic'] ?>"> 
<H3><?php echo $v['title'] ?></H3></A></DIV>
<?php } ?>
</DIV>
<DIV class="item i_length"><!-- start /WEBCONTROLS/SP/SPED_ITEM_list.ASCX -->
<?php $vn=0;$tablev=syClass("syModel")->syCache(3600)->findSql("select id,tid,sid,title,style,trait,gourl,addtime,hits,htmlurl,htmlfile,litpic,orders,mrank,mgold,isshow,keywords,description,pics,video,bigpic,video1 from dy_article a left join dy_article_field b on (a.id=b.aid) where isshow=1 and tid in(34)  order by id asc  limit 3,6");foreach($tablev as $v){ $v["tid_leafid"]=$sy_class_type->leafid($v["tid"]);$v["n"]=$vn=$vn+1; $v["url"]=html_url("article",$v); $v["title"]=stripslashes($v["title"]); $v["description"]=stripslashes($v["description"]); ?>
<DIV class=list><A 
href="<?php echo $v['url'] ?>"><IMG 
alt="<?php echo $v['title'] ?>" src="<?php echo $v['litpic'] ?>"> 
<H3><?php echo $v['title'] ?></H3></A></DIV>
<?php } ?>
</DIV></DIV></DIV></DIV><!--END STROY ITEM--></DIV><!--END ARTICLE--><!--GALLERY--><!--GALLERY--><!--END CONTENT LOADER--></DIV><!--END CONTENT SIDE--><BR 
clear=all></DIV>
<!--FOOTER-->
<DIV id=footer>
<!--HIGHLIGHTS-->
